...
for (p=0; p<number_of_tif; p++) {																						// Beginning of main analysis loop
	...
	while (good_job_edge_detection == false) {																		// Beginning of the edge detection sub-loop
		...
		Dialog.create("FiloQuant: Cell edges detection");																// GUI: create dialog box
		Dialog.addMessage("FiloQuant: Settings");																		// GUI comment
		Dialog.addNumber("Threshold for cell edges:", 40);															// Input threshold_cell_edges
		...
		Dialog.addCheckbox("Fill holes on edges?", false);															// Input HoleEdge
		Dialog.addCheckbox("Fill holes?", false);																			// Input HoleFill
		...
		if (HoleEdge == true) {																									// Amazing trick to close the holes at the edge without blanking the entire image! :D
			...
			run("Canvas Size...", "width="+(w+1)+" height="+(h+1)+" position=Top-Left");			// Reframe the image with line of 1 pixel for padding top left
			...
			run("Canvas Size...", "width="+(w+1)+" height="+(h+1)+" position=Bottom-Right");	// Reframe the image with line of 1 pixel for padding bottom right
			run("Make Binary");																	// Reset image type
			run("Invert");																		// Turns the mask back into ones																																			
			run("Fill Holes");																	// Fill in the holes within the mask
	...
	while (good_job_filopodia_detection == false) {																	// Beginning of the filopodia detection sub-loop
		...
		Dialog.addNumber("Filopodia minimum size:", 50);															// Input for filopodia_min_size
		...
		Dialog.addCheckbox("Use local contrast enhancement to improve filopodia detection ?", true); //  Input for use_clahe
		Dialog.show();																												// Display GUI
	...
	run("Merge Channels...", "c1=[Tagged skeleton] c4=[merge] keep");									// Create a merged composite to inspect accuracy of extraction
	...
	while (user_happy_extraction_check == true) {																	// Beginning of the results saving sub-loop as long as user is happy about everything
		...
		run("Options...", "iterations=60 count=1 black pad do=Close");										// Run the Close command (no user input, hard wired)
		run("Options...", "iterations=10 count=1 black pad do=Erode");										// Run the Erode command (no user input, hard wired)
		run("Options...", "iterations=10 count=1 black pad do=Dilate");										// Run the Dilate command (no user input, hard wired)
		...
		setResult("Settings", 0, "Threshold for cell edges") ;														// Save the header for threshold value used for cell edges
		setResult("Value", 0, threshold_cell_edges) ;																	// Save the threshold value used for cell edges
		setResult("Settings", 1, "Number of iterations for Open") ;												// Save the header for value of open cycles used
		setResult("Value", 1, n_iterations_open) ;																		// Save the value of open cycles used
		setResult("Settings", 2, "Number of iterations for Erode") ;												// Save the header for value of erode cycles used
		setResult("Value", 2, n_cycles_erode_dilate) ;																	// Save the value of erode cycles used
		setResult("Settings", 3, "Fill edges?") ;																			// Save the header for use of Fill Edges command
		setResult("Value", 3, HoleEdge) ;																					// Save the header for use of Fill Edges command
		setResult("Settings", 4, "Fill Holes?") ;																			// Save the header for use of Fill Holes command
		setResult("Value", 4, HoleFill) ;																						// Save the header for use of Fill Holes command
		setResult("Settings", 5, "Threshold for filopodia") ;															// Save the header for threshold value used for filopodia
		setResult("Value", 5, filopodia_threshold) ;																		// Save the header for threshold value used for filopodia
		setResult("Settings", 6, "Filopodia minimum size") ;														// Save the header for minimum value used for filopodia size
		setResult("Value", 6, filopodia_min_size) ;																		// Save the header for minimum value used for filopodia size
		setResult("Settings", 7, "Use convolve to improve filopodia detection?") ;							// Save the header for use of convolution for filopodia detection
		setResult("Value", 7, use_convolve) ;																				// Save the header for use of convolution for filopodia detection
		setResult("Settings", 8, "Use local contrast enhancement to improve filopodia detection?");// Save the header for use of CLAHE
